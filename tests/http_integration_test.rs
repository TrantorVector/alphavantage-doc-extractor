...
async fn test_fetch_alpha_vantage_documentation() {
    ...
    assert!(
        result.is_ok(),
        "Failed to fetch Alpha Vantage documentation: {:?}",
        result.err()
    );
    ...
    assert!(
        html.content.len() > 10000,
        "Content size {} is not > 10000 bytes",
        html.content.len()
    );

    // Verify content contains "Alpha Vantage" and "API"
    assert!(
        html.content.contains("Alpha Vantage"),
        "Content does not contain 'Alpha Vantage'"
    );
    assert!(
        html.content.contains("API"),
        "Content does not contain 'API'"
    );
}
